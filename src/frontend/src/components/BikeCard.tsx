...
export function BikeCard({
  ...
  const handleBuyNow = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (bike.buyUrl) {
      // Use buyUrl redirect with auth gate
      if (user) {
        openBuyUrl(bike.buyUrl);
      } else {
        setPendingUrl(bike.buyUrl);
        setShowAuthModal(true);
      }
    } else {
      // Fallback: open lead form
